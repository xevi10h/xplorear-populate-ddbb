import PopulatePlaceByNameDTO from "./PopulatePlaceByNameDTO";
...
class PopulatePlaceByNameUseCase {
  constructor(
    private readonly placeService: PlaceService,
    private readonly mediaService: MediaService
  ) {}

  async execute({ name, addMedia }: PopulatePlaceByNameDTO): Promise<Place> {
    try {
      ...
      const placeCreated = await this.placeService.createOne(new Place(place));
...
export default PopulatePlaceByNameUseCase;
